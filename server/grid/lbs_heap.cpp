...
int lbs_nnheap_insert(lbs_nnheap_t* lbs_nnheap, lbs_mov_node_t* lbs_mov_node, int cell_id, uint8_t is_grid, double distance)
{
	if(!( lbs_nnheap->size < lbs_nnheap->capacity ))
	{
		if(lbs_nnheap->heap_nodes)
		{
			auto new_ptr = (lbs_heapnode_t*) realloc( lbs_nnheap->heap_nodes, 
			(lbs_nnheap->capacity+10)*sizeof(lbs_heapnode_t));
...
void lbs_nnheap_MinHeapify(lbs_nnheap_t* lbs_nnheap, int i )
{
	...
	if( l<lbs_nnheap->size && lbs_nnheap->heap_nodes[l].distance < lbs_nnheap->heap_nodes[i].distance )
	{
		smallest = l;
	}
	if( r<lbs_nnheap->size && lbs_nnheap->heap_nodes[r].distance < lbs_nnheap->heap_nodes[smallest].distance )
	{
		smallest = r;
	}
